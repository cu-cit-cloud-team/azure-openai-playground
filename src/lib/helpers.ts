...
export const poeticForms: PoeticForm[] = [
  {
    type: 'Ballad',
    rules: `A ballad is similar to an epic in that it tells a story, but it's much shorter and a bit more structured. This poetry form is made up of four-line stanzas (as many as are needed to tell the story) with a rhyme scheme of ABCB. Ballads were originally meant to be set to music, which is where we get the idea of our slow, sultry love song ballads today. A lot of traditional ballads are all in dialogue, where two characters are speaking back and forth.
`,
    example: oneLineTrim`
  ...
  {
    type: 'Cinquain',
    rules: `A cinquain is a five-line poem consisting of twenty-two syllables: two in the first line, then four, then six, then eight, and then two syllables again in the last line. These are deceptively simple poems with a lovely musicality that make the writer think hard about the perfect word choices.`,
    example: oneLineTrim`
      Listen…
  ...
  {
    type: 'Triolet',
    rules: `A triolet is a traditional French single-stanza poem of eight lines with a rhyme scheme of ABAAABAB; however, it only consists of five unique lines. The first line is repeated as the fourth and seventh line, and the second line is repeated as the very last line. Although simple, a well-written triolet will bring new depth and meaning to the repeated lines each time.`,
    example: oneLineTrim`
  ...
  {
    type: 'Dizain',
    rules: `A dizain is another traditional form made up of just one ten-line stanza, and with each line having ten syllables (that's an even hundred in total). The rhyme scheme for a dizain is ABABBCCDCD. This poetry type was a favorite of French poets in the 15th and 16th century, and many English poets adapted it into larger works.`,
    example: oneLineTrim`
  ...
  {
    type: 'Palindrome',
    rules: `Palindrome poems, also called "mirror poems," are poems that begin repeating backwards halfway through, so that the first line and the last line are the same, the second line and the second-to-last line are the same, and so on. They're a challenging yet fun way to show two sides of the same story.`,
    example: oneLineTrim`
  ...
  {
    type: 'Elegy',
    rules: `An elegy is similar to an ode in that it celebrates a person or idea, but in this instance is the poem centers around something that has died or been lost. There's a tradition among poets to write elegies for one another once another poet has died. Sometimes these are obvious memoriams of a deceased person, and other times the true meaning will be hidden behind layers of symbolism and metaphor. Like the ode, there's no formal meter or rhyme scheme in an elegy, though you can certainly experiment with using them.`,
    example: oneLineTrim`
  ...
  {
    type: 'Haiku',
    rules: `A haiku is a traditional cornerstone of Japanese poetry with no set rhyme scheme, but a specific shape: three lines composed of five syllables in the first line, seven in the second line, and five in the third line. Occasionally, some traditional Japanese haikus won't fit this format because the syllables change when they're translated into English; but when you're writing your own haiku poem in your native language, you should try to adhere to this structure.`,
    example: oneLineTrim`
      Over the wintry
...
`,
  ...
  {
    type: 'Nonce',
    rules: `A nonce poem is a DIY poem structure intended for one-time use to challenge yourself as a writer, or just to try something new. It's a formal, rigid, standardized poetry form that's brand new to the world. For example, you might say, "I'm going to write a poem starting with a three-line stanza, then two four-line stanzas, then another three-line stanza, and each line is going to be eight syllables except the first and last line of the poem which are each going to have eleven syllables, and the last word of every stanza will be true rhymes and the first word of every stanza will be slant rhymes." And then you do it, just to see if you can. Nonce poems are a great way to stretch your creativity and language skills to their limit.`,
    example: oneLineTrim`
